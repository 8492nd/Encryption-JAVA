...
public class DES_ECB {
    ...
    //substitution s-box, one table for each section
    ...
    //pc1 which drops the 8 parity bits in the key
    ...
    //pc2 which generates the genuine round key
    ...
    private static long IP(long src)  { return permute(IP, 64, src);                 }
    private static long FP(long src)  { return permute(FP, 64, src);                 }
    private static long E(int src)    { return permute(E, 32, src&0xFFFFFFFFL);      }
    private static int  P(int src)    { return (int)permute(P, 32, src&0xFFFFFFFFL); }
    private static long PC1(long src) { return permute(PC1, 64, src);                }
    private static long PC2(long src) { return permute(PC2, 56, src);                }
    
	
	//forehead casing methods below
	...
	//encryption methods below - only for ECB now
	public static byte[] encrypt(byte[] msg, byte[] key) {
		byte[] ciphertext = new byte[msg.length];
		for(int i = 0; i < msg.length; i++) encryptBlock(msg, i, ciphertext, i, key);
		return ciphertext;
	}
