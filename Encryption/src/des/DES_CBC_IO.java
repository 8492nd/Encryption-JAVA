...
public class DES_CBC_IO {
    ...
    public static boolean testIO(String addr, String password, byte[] iv) {
        return testIO(addr, passwordToKey(password), iv);
    }
    
    public static boolean testIO(String addr, byte[] key, byte[] iv) {
        System.out.println("IO Test #"+(++testCount)+":");
        System.out.println("\tiv:\t\t"+	hex(iv));
        System.out.println("\tkey:\t\t"+	hex(key));
        //boolean result = Arrays.equals(expected, received) && Arrays.equals(message, plain);
        ...
        try {
        	...
        	byte[] message = new byte[read.length + 8 - read.length%8];
        	System.arraycopy(read, 0, message, 0, read.length);
        	if(read.length%8!=0) {
        		for(int i=0; i<8-read.length%8; i++) {
        			message[read.length+i] = -128;
        		}
        	}
        	input.close();
        	byte[] received = encrypt(message, key, iv);
            byte[] plain = decrypt(received, key, iv);
            byte[] decrypted = new byte[read.length];
            for(int i=0; i<read.length; i++) decrypted[i] = plain[i];
            OutputStream output = new FileOutputStream(address + ".enc");
            OutputStream output2 = new FileOutputStream(address + ".enc.dec");
    	    output.write(received);
    	    output.close();
    	    //output2.write(plain);
    	    output2.write(decrypted);
    	    output2.close();
    ...
    public static byte[] genVal() {
    	String vec = "";
    	Random ran = new Random();
    	int a = 0;
    	for(int i = 0; i < 16; i++) {
    		a = ran.nextInt(0x10);
    		vec += Integer.toHexString(a);
    	}
    	byte[] v = parseBytes(vec);
    	return v;
    }
    ...
    public static void main(String[] args) {
        ...
        testIO(
        	"C:/Users/w10Test/Desktop/Encrypto/DES/plain.txt",
        	"mypass",
            parseBytes("fa60 69b9 85fa 1cf7") //iv
        );
        testIO(
        	"C:/Users/w10Test/Desktop/Encrypto/DES/desio.txt",
        	key,
        	vec
        );
    }
